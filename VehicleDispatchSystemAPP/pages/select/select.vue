	...
	<view class="content" style="min-height: 100vh;">
		<view class="card-info">
			...
			<view class="select-parkade" @click="mapIdx = 1">
				<image :src="baseURL + parkade.img"></image>
				<view class="text">
		...
		<view class="card-list" >
			...
			<view class="card-item" v-for="(item, key) in vehicleCarList" @click="toDetails(item)">
				<image class="show" :src='baseURL + item.modelImg'></image>
				<view class="text">
					<p class="title">{{item.name}}</p>
					<span class="type">{{item.type}}/{{item.driverType}}/{{item.displacement}}/{{item.enerty}}</span>
